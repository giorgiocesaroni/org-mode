...
function App() {
  ...
  function handleRequestPushPermissions() {
    Notification.requestPermission().then(alert);
  }
  ...
  return (
    <div className="pb-4 max-w-lg mx-auto">
      ...
      <div className="text-white grid gap-2 p-2">
        <p>⚠️ Request Push API permissions</p>
        <button
...
function Text({ children }) {
  return (
    <p className="text-gray-500 pb-4 whitespace-pre-line">{children.trim()}</p>
  );
}
...
function Checkbox({ disabled, checked, onClick }) {
  ...
  return (
    <div onClick={!disabled ? onClick : null} className="flex relative w-5 h-5">
      <div
